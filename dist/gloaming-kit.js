var gloamingKit = (() => {
  ...
  // src/signals.js
  var warn = (path, message) => console.warn(`GloamingKit routing (${path}): ${message}`);
  var constant = (k) => () => k;
  var band = (name, path) => {
    if (!(name in BANDS)) {
  ...
  var STYLE_TAU = 0.3;
  var GloamingKit = class extends Emitter {
    constructor({
      canvas,
      style = {},
      timeline = [],
        ...
        if (def.kind !== "event") {
          for (const name2 of referencedTriggers(spec)) {
            if (!this.analyzer.hasTrigger(name2)) {
              console.warn(`GloamingKit: '${id}.${slot}' references trigger '${name2}', which is not configured \u2014 its envelope will stay at 0`);
            }
          }
          continue;
        }
...
//# sourceMappingURL=gloaming-kit.js.map
